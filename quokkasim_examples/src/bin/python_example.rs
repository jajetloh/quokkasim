...
fn bench(_cfg: ()) -> Result<(Simulation, EndpointRegistry), SimulationError> {
    ...
    let inner_process_logger = match process_logger {
        ComponentLogger::Vector3ProcessLogger(logger) => {
            logger
        }
        _ => panic!("Expected Vector3ProcessLogger"),
    }.buffer;
    registry.add_event_sink(inner_process_logger.into_reader(), "add_1_output").unwrap();
